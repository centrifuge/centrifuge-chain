...
pub const LP_SOL_SOURCES: &str = env!("LP_SOL_SOURCES", "Build script failed to populate environment variable LP_SOL_SOURCES pointing to missing solidity source files in the 'target/*/build/integration-tests*/out' directory.");
...
pub fn deploy_contract<T: Runtime>(address: impl AsRef<[u8]> + Clone, code: Vec<u8>) -> H160 {
	...
	let derived_address =
		AccountConverter::convert_evm_address(chain_id, vec_to_fixed_array(address.clone()));
	...
	pallet_evm::Pallet::<T>::create(
		T::RuntimeOriginExt::signed(derived_address),
