...
use sp_api::{ApiExt, BlockT, ConstructRuntimeApi};
...
pub trait FudgeHandle {
	...
	type ParachainRuntime: Runtime<Block = Self::ParachainBlock>;

	type ParachainBlock: BlockT;

	type ParachainConstructApi: ConstructRuntimeApi<
			Self::ParachainBlock,
			ParachainClient<Self::ParachainBlock, Self::ParachainConstructApi>,
			RuntimeApi = Self::ParachainApi,
		> + Send
		+ Sync
		+ 'static;

	type ParachainApi: BlockBuilderApi<Self::ParachainBlock>
		+ ApiExt<
			Self::ParachainBlock,
			StateBackend = <TFullBackend<Self::ParachainBlock> as Backend<Self::ParachainBlock>>::State,
        > + AuraApi<Self::ParachainBlock, AuthorityId>
		+ TaggedTransactionQueue<Self::ParachainBlock>;
	...
	fn parachain(&self) -> &ParachainBuilder<Self::ParachainBlock, Self::ParachainConstructApi>;
	fn parachain_mut(
		&mut self,
	...
	fn build_parachain(
		...
		storage: Storage,
	) -> ParachainBuilder<Self::ParachainBlock, Self::ParachainConstructApi> {
		...
		state.insert_storage(
			...
			.build_storage::<Self::ParachainRuntime>()
			.unwrap(),
		);
		state.insert_storage(
			pallet_aura::GenesisConfig::<Self::ParachainRuntime> {
