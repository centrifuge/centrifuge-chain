...
fn test_anchor_migration() {
    ...
    new_test_ext().execute_with(|| {
        ...
        // rc6 - "anchor" + days_rent.enc() + "anchor" + days_rent.enc()  + anchorID

        // Storagekey - ":child_storage:default:" + days_rent.enc() [18867.enc()]
        let storage_key: &[u8] =
            &[58, 99, 104, 105, 108, 100, 95, 115, 116, 111, 114, 97, 103, 101, 58, 100, 101, 102, 97, 117, 108, 116, 58, 179, 73, 0, 0];
        // Child Def
        // let child_prefix: &[u8] = &[97, 110, 99, 104, 111, 114];
        // Key - AnchorID
        let child_key: &[u8; 32] = &[241, 21, 217, 102, 123, 102, 236, 112, 165, 212, 213, 131, 149, 180, 145, 99, 243, 158, 65, 201, 198, 9, 83, 69, 109, 182, 146, 201, 235, 159, 32, 234];

        let raw_doc_root: &[u8; 32] = &[107, 84, 243, 195, 50, 123, 210, 7, 187, 43, 195, 161, 54, 218, 203, 82, 230, 75, 204, 83, 51, 29, 27, 125, 187, 36, 110, 3, 250, 118, 128, 186];
        ...
        assert_eq!(Anchor::run_anchor_migration(), 1);
