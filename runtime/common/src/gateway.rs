...
pub struct GatewayAccountProvider<T>(core::marker::PhantomData<T>);

impl<T> GatewayAccountProvider<T>
where
	T: pallet_evm_chain_id::Config + parachain_info::Config,
{
	pub fn get_gateway_account() -> AccountId {
		...
		AccountConverter::<T>::into_account_id(truncated_sender_account)
	}
}
