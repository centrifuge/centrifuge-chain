...
/// It can be used when you need to ask to a third party or external module if applying a change
/// that has some effect into the system is something healthy.
pub trait ChangeGuard {
	...
	/// The caller to this method ask for feedback for the implementation of this trait in order
	/// be able to semantically proceed successful with that change.
	/// The change intention will be noted by this method and identified by the returned ChangeId.
	...
	/// - The change is not ready to be applied yet. The conditions not fulfilled.
	...
	/// The change can no longer be released, and resources to track it can be removed.
