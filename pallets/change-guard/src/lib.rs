...
//! Handle changes that require an special treatment to only release them once
...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		/// A change identification
		type Change: Parameter + MaxEncodedLen + Ord + Default;
	}

	/// Storage that contains the registering information
	#[pallet::storage]
	pub(crate) type Changes<T: Config> =
		StorageDoubleMap<_, Blake2_128Concat, T::PoolId, Blake2_128Concat, T::ChangeId, T::Change>;
