...
use cfg_types::{
	pools::{FeeType, PoolFee, TrancheMetadata},
...
benchmarks! {
	...
	where
		...
		<T as Config>::ModifyPool: PoolMutate<
			...
			PoolFeeInput = (FeeBucket, PoolFee<<T as frame_system::Config>::AccountId, FeeType<<T as pallet_pool_system::Config>::Balance, <T as pallet_pool_system::Config>::Rate>>),
		>,
	}
	register {
