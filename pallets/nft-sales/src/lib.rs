//! NFT Sales pallet
//!
//! This pallet provides a place for digital art creators and owners to offer
//! their NFTs for sale and for potential buyers to browse and buy them.
//!
//! To sell NFTs, users will call [`Pallet::add()`]. Doing so will have the NFT
//! being transferred from the seller to this pallet's account, to grant us the
//! own
//!
//! To remove an NFT from sale and thus reclaim its ownership, sellers will call
//! [`Pallet::remove()`].
//!
...
pub mod pallet {
	...
	pub enum Error<T> {
		...
		/// A buyer's max offer is invalid, i.e., either the currency or amount
	...
	impl<T: Config> Pallet<T> {
		...
		///     - Err(NotFound) when the NFT could not be found in
		///       [`Config::NonFungibles`]
		fn is_owner(
			account: T::AccountId,
			class_id: T::CollectionId,
