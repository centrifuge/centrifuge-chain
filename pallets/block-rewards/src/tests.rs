...
fn check_special_privileges() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			BlockRewards::set_distributed_reward(RuntimeOrigin::signed(USER_A), 10),
			BadOrigin
		);
		assert_noop!(
			BlockRewards::set_epoch_duration(RuntimeOrigin::signed(USER_A), 100),
			BadOrigin
		);
		assert_noop!(
			BlockRewards::set_group_weight(RuntimeOrigin::signed(USER_A), GROUP_A, 3),
			BadOrigin
		);
		assert_noop!(
			BlockRewards::set_currency_group(RuntimeOrigin::signed(USER_A), CURRENCY_ID_A, GROUP_A),
			BadOrigin
		);
	});
}
...
fn distributed_reward_change() {
	...
	new_test_ext().execute_with(|| {
		// EPOCH 0
		assert_ok!(BlockRewards::set_distributed_reward(
			RuntimeOrigin::root(),
			REWARD
		));
		...
		BlockRewards::on_initialize(INITIAL_EPOCH_DURATION);

		// EPOCH 1
		assert_eq!(NextEpochChanges::<Test>::get().reward, None);
		assert_eq!(ActiveEpochData::<Test>::get().reward, REWARD);

		BlockRewards::on_initialize(INITIAL_EPOCH_DURATION + INITIAL_EPOCH_DURATION);
...
fn epoch_change() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_ok!(BlockRewards::set_epoch_duration(
			RuntimeOrigin::root(),
			EPOCH_DURATION
		));
		assert_eq!(
		...
		BlockRewards::on_initialize(INITIAL_EPOCH_DURATION + EPOCH_DURATION / 2);
		...
		BlockRewards::on_initialize(INITIAL_EPOCH_DURATION + EPOCH_DURATION);
...
fn epoch_change_from_advanced_state() {
	...
	new_test_ext().execute_with(|| {
		...
		BlockRewards::on_initialize(SYSTEM_BLOCK_NUMBER);
...
fn currency_changes() {
	...
	new_test_ext().execute_with(|| {
		// EPOCH 0
		assert_ok!(BlockRewards::set_currency_group(
			RuntimeOrigin::root(),
			CURRENCY_ID_A,
			GROUP_A
		));
...
fn weight_changes() {
	...
	new_test_ext().execute_with(|| {
		...
		// EPOCH 1
		assert_ok!(BlockRewards::set_group_weight(
			RuntimeOrigin::root(),
			GROUP_A,
			WEIGHT_1
		));
		...
		BlockRewards::on_initialize(INITIAL_EPOCH_DURATION * 2);
		...
		BlockRewards::on_initialize(INITIAL_EPOCH_DURATION * 3);
	});
}
...
fn max_currency_changes() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			BlockRewards::set_currency_group(
				RuntimeOrigin::root(),
				MaxChangesPerEpoch::get() + 1,
				100
			),
