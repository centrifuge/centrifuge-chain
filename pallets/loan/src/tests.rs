...
type MultiCurrencyBalanceOf<T> = <T as pallet_pool::Config>::Balance;
...
{
	<T as pallet_pool::Config>::Tokens::total_balance(currency_id, account)
}

fn issue_test_loan<T>(pool_id: u64, borrower: T::AccountId) -> (T::PoolId, AssetOf<T>, AssetOf<T>)
where
	T: pallet_pool::Config<
	...
	PoolIdOf<T>: From<<T as pallet_pool::Config>::PoolId>,
{
	...
	assert_ok!(pallet_pool::Pallet::<T>::approve_role_for(
	...
	($price_loan:ident, $maturity_check:expr) => {
		TestExternalitiesBuilder::default()
			...
			.execute_with(|| {
				...
				assert_ok!(pallet_pool::Pallet::<MockRuntime>::approve_role_for(
				...
				let res =
					Loan::admin_write_off_loan(Origin::signed(risk_admin), pool_id, loan_id, 0);
				assert_ok!(res);
	...
	($price_loan:ident,$moving_ceiling:expr,$admin_write_off:expr,$pv_1:expr,$pv_200:expr) => {
		TestExternalitiesBuilder::default()
			...
			.execute_with(|| {
				...
				if $admin_write_off {
					let res =
						Loan::admin_write_off_loan(Origin::signed(risk_admin), pool_id, loan_id, 2);
					assert_ok!(res);
				} else {
		...
		TestExternalitiesBuilder::default()
			...
			.execute_with(|| {
				...
				Timestamp::set_timestamp(
					math::seconds_per_year() * 2 + 5 * math::seconds_per_day(),
				);
				let res = Loan::close_loan(Origin::signed(borrower), pool_id, loan_id);
				assert_err!(res, Error::<MockRuntime>::ErrLoanNotRepaid);
				...
				if $maturity_checks {
					...
					Timestamp::set_timestamp(
						math::seconds_per_year() * 2 + 120 * math::seconds_per_day(),
					);
					let res = Loan::write_off_loan(Origin::signed(200), pool_id, loan_id);
					assert_ok!(res);
				} else {
					// write off as admin
					let res =
						Loan::admin_write_off_loan(Origin::signed(risk_admin), pool_id, loan_id, 4);
					assert_ok!(res);
				}
