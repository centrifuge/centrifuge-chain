...
fn send_sibling_asset_to_reserve_sibling() {
	...
	ParaA::execute_with(|| {
		assert_eq!(
			ParaTokens::free_balance(CurrencyId::A, &sibling_b_account()),
			500
		);
...
fn send_sibling_asset_to_reserve_sibling_with_distinct_fee() {
	...
	ParaB::execute_with(|| {
		...
		assert_eq!(
			ParaTokens::free_balance(CurrencyId::B1, &sibling_a_account()),
			950
		);
...
fn send_sibling_asset_to_non_reserve_sibling() {
	...
	ParaB::execute_with(|| {
		...
		assert_eq!(
			ParaTokens::free_balance(CurrencyId::B, &sibling_c_account()),
			460
		);
	});
...
fn send_self_parachain_asset_to_sibling_with_distinct_fee() {
	...
	ParaA::execute_with(|| {
		...
		assert_eq!(
			ParaTokens::free_balance(CurrencyId::A1, &sibling_b_account()),
			50
		);
	});
...
fn sending_sibling_asset_to_reserve_sibling_with_relay_fee_works() {
	...
	ParaC::execute_with(|| {
		assert_ok!(ParaTeleportTokens::deposit(
			CurrencyId::C,
			&sibling_a_account(),
			1_000
		));
	...
	ParaA::execute_with(|| {
		...
		assert_eq!(
			1000 - fee_amount,
			ParaTokens::free_balance(CurrencyId::R, &ALICE)
		);
	});
	...
	ParaA::execute_with(|| {
		assert_noop!(
			ParaXTokens::transfer_multiasset(
				...
				Box::new(
					(
						X1(GeneralKey(b"B".to_vec().try_into().unwrap())).into(),
						100
					)
						.into()
				),
	...
	ParaA::execute_with(|| {
		assert_noop!(
			ParaXTokens::transfer_multiasset(
				Some(ALICE).into(),
				Box::new(
					MultiAsset::sibling_parachain_asset(1, b"A".to_vec().try_into().unwrap(), 100)
						.into()
				),
				Box::new(
					MultiLocation::new(
...
fn send_with_insufficient_fee_traps_assets() {
	...
	ParaB::execute_with(|| {
		assert!(para::System::events().iter().any(|r| {
			matches!(
				r.event,
				para::Event::PolkadotXcm(pallet_xcm::Event::<para::Runtime>::AssetsTrapped(
					_,
					_,
					_
				))
...
fn send_sibling_asset_to_reserve_sibling_with_relative_view() {
	...
	ParaD::execute_with(|| {
		assert_ok!(ParaRelativeTokens::deposit(
			CurrencyId::D,
			&sibling_a_account(),
			1_000
		));
	...
	ParaA::execute_with(|| {
		assert_eq!(
			ParaTokens::free_balance(CurrencyId::A, &sibling_d_account()),
			500
		);
		assert_eq!(ParaTokens::free_balance(CurrencyId::A, &ALICE), 460);
	});
