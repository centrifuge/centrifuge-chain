...
pub fn setup_nft(owner: u64, token_id: U256, resource_id: ResourceId) -> RegistryId {
    ...
    let (asset_id,
            ...
            nft_data,
            _) = pallet_registry::mock::setup_mint::<MockRuntime>(owner, token_id);

    // Commit document root
    assert_ok!( 
        <pallet_anchors::Pallet<MockRuntime>>::commit(
            origin.clone(),
            pre_image,
            doc_root,
    ...
    assert_ok!(<chainbridge::Pallet<MockRuntime>>::register_resource(resource_id.clone(), vec![]));

    // Register resource in local resource mapping
    <pallet_bridge_mapping::Pallet<MockRuntime>>::set_resource(resource_id.clone(),registry_id.clone().into());
