...
//        + chainbridge::Config
	...
	impl<T:Config> Pallet<T> {
        ...
        pub fn transfer_asset(
            ...
            let resource_id = <pallet_bridge_mapping::Pallet<T>>::name_of(reg).ok_or(Error::<T>::ResourceIdDoesNotExist)?;
        ...
        pub fn transfer_native(
            ...
            let currency_token_fee: BalanceOf<T> = 
                TryInto::<u128>::try_into(token_fee)
                    .map_err(|_| Error::<T>::TokenTransferFeeNotConvertibleToCurrency)?
                        .try_into()
                            .map_err(|_| Error::<T>::TokenTransferFeeNotConvertibleToCurrency)?;
            let total_amount = amount.checked_add(&currency_token_fee).ok_or(Error::<T>::TotalAmountNotConvertibleToCurrency)?;
            ...
            let remaining_balance = <T as pallet::Config>::Currency::free_balance(&source).checked_sub(&total_amount).ok_or(Error::<T>::InsufficientBalance)?;
            ...
            <pallet_fees::Pallet<T>>::burn_fee(&source, T::TokenTransferFee::get().saturated_into())?;
