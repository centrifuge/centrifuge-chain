...
fn transfer_native() {
    TestExternalitiesBuilder::default().build().execute_with(|| {
        ...
        let mut account_current_balance = <pallet_balances::Pallet<MockRuntime>>::free_balance(RELAYER_B);
...
fn transfer_nonfungible_asset() {
    TestExternalitiesBuilder::default().build().execute_with(|| {
        ...
        // Owner owns nft
        assert_eq!(<pallet_nft::Pallet<MockRuntime>>::account_for_asset(registry_id, token_id),
                    Some(owner));
...
fn execute_remark_bad_origin() {
    TestExternalitiesBuilder::default().build().execute_with(|| {
        ...
        assert_ok!(Bridge::remark(Origin::signed(Chainbridge::account_id()), hash, r_id));
...
fn transfer() {
    TestExternalitiesBuilder::default().build().execute_with(|| {
        ...
        assert_ok!(
            Bridge::transfer(Origin::signed(Chainbridge::account_id()),
            RELAYER_A,
            10,
            resource_id
        ));
        ...
        assert_events(vec![
            mock::Event::Balances(pallet_balances::Event::Transfer(Chainbridge::account_id(), RELAYER_A, 10)),
        ]);
    })
}
