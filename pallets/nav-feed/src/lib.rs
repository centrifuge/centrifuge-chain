//! # Nav-feed pallet for runtime
//!
//! This pallet provides functionality for managing/calculating/computing Pool value
...
pub struct LoanInfo<RiskGroup, Balance> {
...
impl<RiskGroup, Balance> LoanInfo<RiskGroup, Balance>
where
	Balance: PartialOrd + sp_arithmetic::traits::Zero,
{
...
pub mod pallet {
	...
	pub(super) type PoolValue<T: Config> =
		StorageMap<_, Blake2_128Concat, T::PoolID, BalanceOf<T>, OptionQuery>;
	...
	pub(super) type Loan<T: Config> = StorageDoubleMap<
		...
		Blake2_128Concat,
		T::LoanID,
		LoanInfo<T::RiskGroup, BalanceOf<T>>,
		OptionQuery,
	>;
	...
	pub enum Event<T: Config> {
		...
		LoanAmountBorrowed(T::PoolID, T::LoanID, BalanceOf<T>),
	}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn set_loan_info(
			origin: OriginFor<T>,
			pool_id: T::PoolID,
			loan_id: T::LoanID,
			risk_group: T::RiskGroup,
			rate: BalanceOf<T>,
			...
			Loan::<T>::mutate(pool_id, loan_id, |maybe_loan_info| {
				...
				loan_info.risk_group = risk_group;
				loan_info.principal = principal;
				loan_info.rate = rate;
...
impl<T: Config> Pallet<T> {
	...
	pub fn pool_value(pool_id: T::PoolID) -> Option<BalanceOf<T>> {
		PoolValue::<T>::get(pool_id)
	}

	/// returns the ceiling of the given loan under a given pool.
	pub fn ceiling(pool_id: T::PoolID, loan_id: T::LoanID) -> Option<BalanceOf<T>> {
	...
	pub fn borrow(pool_id: T::PoolID, loan_id: T::LoanID, amount: BalanceOf<T>) -> DispatchResult {
		let loan_info = Loan::<T>::get(pool_id, loan_id).ok_or(Error::<T>::ErrMissingLoan)?;
		ensure!(
		...
		let new_ceiling = loan_info
			.ceiling
			.checked_sub(&amount)
			.ok_or(Error::<T>::ErrSubCeilingUnderflow)?;

		let borrowed_amount = loan_info.borrowed_amount;

		Loan::<T>::mutate(pool_id, loan_id, |maybe_loan_info| {
			let mut loan_info = maybe_loan_info.take().unwrap_or_default();
			loan_info.borrowed_amount = new_borrowed_amount;
			loan_info.ceiling = new_ceiling;
			*maybe_loan_info = Some(loan_info);
		});

		if borrowed_amount == Zero::zero() {
